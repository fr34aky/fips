...
# dead within weeks.
#
# Kept on this fork as a developer tool: CI does not use it, since the
# FreeBSD 16 package is the FreeBSD 15.1 build relabelled (see
# packaging/pfsense/README.md). It still boots a plain FreeBSD 16 when one is
# wanted, on a GitHub runner or a developer machine (tested on Arch with 4 GB
# for the VM: VM_MEM_MB=4096 FREEBSD16_VM_STATE=~/.cache/fips-freebsd-vm).
# Every `up` re-fetches and re-decompresses the image, so nothing inside the
# VM survives between runs. Pin, mirror and refresh procedure are as the
# archive/pfsense-freebsd16-builder branch describes.
#
...
cmd_up() {
    ...
    # CI starts from an empty RUNNER_TEMP; a developer machine does not, and
    # ssh-keygen stops at an overwrite prompt if the previous run's key is
    # still there.
    rm -f "$STATE/console.log" "$STATE/console.sock" "$STATE/known_hosts" \
          "$STATE/id_ed25519" "$STATE/id_ed25519.pub"
