...
/// on a connection-oriented transport, on the rx loop; retrying that every tick
/// would make a stranded stream a stalled node.
        ...
        for action in actions {
            match action {
                ...
                MmpAction::Heartbeat { peer } => {
                    // Attempt first, success after: the attempt is recorded even
                    // if the send below fails or never returns, so the retry
                    // stays spaced; only a send that came back clean moves the
                    // interval that says the peer has heard from us.
